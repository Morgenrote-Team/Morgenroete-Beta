Changelog fot 2.7.0l "Pasteur"

Art:
- Replaced an AI loading screen with one by Angel depicting the construction of a Viaduct of the Schöllenenbahn (CC4.0 BY-NC angel): https://x.com/angelsrntreal/status/2029535306220011934

Characters:
- Jules Bordet (random Belgian Biologist and Physician)
- Barthélemy Charles Joseph Dumortier (random Belgian Biologist)
- Octave Gengou (random Belgian Biologist and Physician)
- André Gratia (random Belgian Biologist and Phsysician)
- Émile van Ermengem (random Belgian Biologist)
- Évariste Warlomont (random Belgian Biologist and Physician)
- Louis Willems (random Belgian Biologist)
- Louis Berckmans (random Belgian Physician and Biologist)
- Léon Fredericq (random Belgian Physician and Biologist)
- Ferdinand Augustijn Snellaert (random Belgian Physician)
- Chalerm Buatang (Random Thai Composer)
- Cham Sunthornwathin (Random Thai Composer)
- Chit Chittasewi (Random Thai Composer)
- Choi Sunthornwathin (Random Thai Composer)
- Christopher Hewetson (Random British Composer)
- Chuangchan Chankana (Random Thai Composer & Writer)
- Eua Sunthornsanan (Random Thai Composer)
- Mi Duriyangkul (Random Thai Composer)
- Montri Tramote (Random Thai Composer)
- Piti Wathayakon (Random Thai Composer)
- Plaek Prasansap (Random Thai Composer)
- Pyotr Shchurovsky (Random Russian Composer)
- Sorn Silpabanleng (Random Thai Composer)
- Thewaprasit Parttayakason (Random Thai Composer)
- Tua Parttayakason (Random Thai Composer)
- Un Duriyachiwin (Random Thai Composer)
- Chote Suvatti (random Thai naturalist)
- Kong Thawornwet (random Thai physician)
- Uai Ketsing (random Thai biologist & physician)
- Burachat Chaiyakorn (random Thai Engineer & Geologist)
- Carlo Allegri (random North Italian Engineer)
- Charoen Chenakul (random Thai Engineer)
- Emil Eisenhofer (random South German Engineer)
- Frederick Henry Smiles (random Scottish Engineer)
- John Edward Austin Shea (random Irish Engineer)
- Henry Gittins (random English Engineer)
- Loen Pongsopon (random Thai Pilot & Engineer)
- Munee Mahasantana (random Thai Engineer)
- Thep Phanthumsen (random Thai Engineer)
- Charles van den Born (random Wallonian Pilot & Racer)
- Dick Farman (random British Engineer)
- Maurice Farman (random British Engineer, Pilot, & Racer)
- Henri Farman (random British Engineer, Pilot, & Racer)
- Ernest Archdeacon (random French Pilot & Racer)
- Lawrence Hargrave (random Australian Engineer & Astronomer)
- Fuen Ronnaphagrad Ritthakhanee (random Thai Pilot)
- Boonjeam Komolmit (random Thai Pilot)
- György Jendrassik (random Hungarian Engineer)
- Jira Wichitsongkhram (random Thai Engineer)
- Wichian Suttantanon (random Thai Engineer)
- Léon Delagrange (random French Engineer, Pilot, and Artist)
- Alessandro Anzani (random North Italian Engineer & Racer)
- Charles Renard (random French Engineer)
- Henri Giffard (random French Engineer)
- Eugène Bourdon (random French Engineer)
- Lucien Vidie (random French Engineer)
- Gustave Zédé (random French Engineer)
- Arthur Constantin Krebs (random French Engineer)

Changes:
- Trying to find a Pathogen will no longer require you to spend the Microorganisms (as this made it too hard to find them).
- Ronald Ross and Élie Metchnikoff can now spawn as a Biologist.
- non historical Biologists now start with a specialization (Botanist, Zoologist, Microbiologist, Epidemiologist)

Fixes:
- You no longer get more than 1000 mechanical points.
- The balloonist should no longer be able to get more than 300 experience.

Performance:
- Improved performance of the mechanical points (they are now added half yearly instead of monthly).
- Improved performance of the full of lies journal entry by removing the any_scope_pop trigger.
- Improved performance of ballooning events by only firing them half yearly.
- Various small performance improvements.
